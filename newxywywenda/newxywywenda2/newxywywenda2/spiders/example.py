...
class ExampleSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        for i in range(1, 32):
            ...
            url_d = "http://club.xywy.com/keshi/2007-10-{}/1.html".format(i)
    ...
    def parse_disease(self,response):
        ...
        "").replace("请问医生，","").replace("。，","。").replace("输入您的问题","").replace("\ue5e5","").replace("\\ue5e5","")
        if len(question) >1:
        ...
        if answer_list is not None:
            ...
            for an in answer_list:
                ...
                "").replace("。，","。").replace("您好！","").replace("您好!","").replace("\ue5e5","").replace("\\ue5e5","")
                if len(answer) > 2:
                    answer_f = answer + ","
