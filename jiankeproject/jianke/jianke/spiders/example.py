# -*- coding: utf-8 -*-
import scrapy
from scrapy.http import Request
import re
from copy import deepcopy

#//a/following-sibling::*
class ExampleSpider(scrapy.Spider):
    name = 'jieba_try'
    allowed_domains = ['www.jianke.com']
    start_urls = ['https://www.jianke.com/ask/browse/0-1-1']


    def parse(self, response):
        item = {}
        li_list = response.xpath("//div[@class='classbox blue_br m_b10']/ul/li")
        for li in li_list:
            url_raw = li.xpath("./a/@href").extract_first()
            text = li.xpath("./a/@title").extract_first()
            item["class"]  = text
            url = "https://www.jianke.com" + url_raw

            yield Request(url,callback=self.parse_jieda,meta={"item":deepcopy(item)},dont_filter=True)

    def parse_jieda(self,response):
        item = response.meta["item"]

        url_raw = response.xpath("//ul[@class='tabTag']//a[text()='已解答']/@href").extract_first()
        url = "https://www.jianke.com" + url_raw
        # print(url)

        yield Request(url,callback=self.parse_liebiao,meta={"item":deepcopy(item)},dont_filter=True)


    def parse_liebiao(self,response):
        item = response.meta["item"]

        li_list = response.xpath("//ul[@class='alltw_box']/li")
        for li in li_list:
            url_raw = li.xpath("./dl/dd[1]/span/a/@href").extract_first()
            #问题的url地址
            url = "https://www.jianke.com" + url_raw

            small_class = li.xpath("./dl/dd[1]/span/strong/a/text()").extract_first()
            item["small_class"] = small_class

            yield Request(url,callback=self.parse_disease,meta={"item":deepcopy(item)},dont_filter=True)

        next_url_raw = response.xpath("//a[text()='下一页']/@href").extract_first()
        # print(next_url_raw)
        if next_url_raw is not None:
            next_url = "https://www.jianke.com" + next_url_raw
            # print(next_url)
            yield Request(next_url,callback=self.parse_liebiao,meta={"item":deepcopy(item)},dont_filter=True)


    def parse_disease(self,response):
        item = response.meta["item"]

        title_raw = response.xpath("//div[@class='why']/h1/text()").extract_first()
        item["title"] = title_raw

        question_raw = response.xpath("//div[@class='ask fl']/p[@class='pd_txt']/text()").extract_first()
        item["question"] = question_raw
        # print(question_raw)

        answer_raw = response.xpath("//div[@class='anwsernew fl']/div[@class='an_cont']/dl/dt/text()").extract()
        answer = str(answer_raw).replace("['","").replace("']","").replace("\\u3000","")
        item["answer"] = answer

        yield item




















    # def parse(self, response):
    #     url_list = []
    #     div_list = response.xpath("//div[@class='mc']/div[4]") #/following-sibling::*    2 3 4 5 6 7  11 12 13
    #     for div1 in div_list:
    #         a_list = div1.xpath("./span/i/s/a")
    #         for a in a_list:
    #             url_raw1 = a.xpath("./@href").extract_first()
    #             url1 = "http://www.jianke.com" + url_raw1
    #             url_list.append(url1)
    #
    #     for div2 in div_list:
    #         dd_list = div2.xpath("./div/dl/dd")
    #         for dd in dd_list:
    #             url2_raw = dd.xpath("./a/@href").extract_first()
    #             url2 =  "http://www.jianke.com" + url2_raw
    #             url_list.append(url2)
    #     for url in url_list:
    #         yield Request(url, callback=self.parse_one, dont_filter=True)
    #
    #
    # def parse_one(self,response):
    #     text = response.xpath("//ul[@class='fenleilist clearfix']/li/a/@title").extract()
    #     print(text)
    #



        # print(len(url_list))# 所有的中等标签
    #     for url  in url_list:
    #         all_url_raw = re.findall('(.*-)\w+',url)
    #         for i in range(20000):
    #             all_url = all_url_raw[0]+str(1)
    #             yield Request(url, callback=self.parse_one, dont_filter=True)
    #
    #
    # def parse_one(self,response):
    #     li_list = response.xpath("//ul[@class='alltw_box']/li")
    #     for li in li_list:
    #         url_raw = li.xpath("./dl/dd[@class='dd1']/span/a/@href").extract_first()
    #         url = "http://www.jianke.com" + url_raw
    #         yield Request(url, callback=self.parse_two, dont_filter=True)
    #
    #
    #
    # def parse_two(self,response):
    #     item = {}
    #     list = [] #title列表
    #     list2 = []#回答列表
    #     a_list = response.xpath("//h5/a[text()='所有问题']/following-sibling::*")
    #     for a in a_list:
    #         title_list = a.xpath("./text()").extract_first()
    #         list.append(title_list)
    #
    #     item["title"] = str(list)
    #
    #     question_title_raw = response.xpath("//div[@class='why']/h1/text()").extract_first()
    #     question_title = str(question_title_raw)
    #     item["question_title"] = question_title
    #
    #     gender_age_list  = response.xpath("//p[@class='name_age f12']/text()").extract()
    #     gender_raw = gender_age_list[0]
    #     gender = str(gender_raw).replace("\r\n","").replace(" ","")
    #     item["gender"] = gender
    #
    #
    #     age_raw = gender_age_list[1]
    #     age = str(age_raw).replace("\r\n", "").replace(" ", "")
    #     item["age"] = age
    #
    #     question_raw = response.xpath("//p[@class='pd_txt']/text()").extract_first()
    #     question = str(question_raw)
    #     item["question"] = question
    #
    #     answer_list = response.xpath("//div[@class='anwsernew fl']")
    #     for answer in answer_list:
    #         answer_raw = answer.xpath("./div[@class='an_cont']/dl/dt//text()").extract()
    #         list2.append(answer_raw)
    #     item["answer"] = str(list2).replace("'","")
    #
    #     yield item
    #
    #
    #
    #
    #



#      （*-*-）






